...
function NextDayTodo() {
  ...
  useEffect(() => {
    ...
    // Check if the current hour is between 5 am and 7 am
    const isBetween5And7 = currentHour >= 6 && currentHour < 7;

    setIsVisible(isBetween5And7);
  }, []);
  ...
  // Filter todos to include only tomorrow's tasks
  const tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate() + 1); // Get tomorrow's date
  const tomorrowDateString = tomorrow.toDateString();
  const filteredTodos = todos.filter(
    (todo) => new Date(todo.date).toDateString() === tomorrowDateString
  );
